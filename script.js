...
function toggleSearch() {
    ...
    if (!overlay.classList.contains('hidden')) {
        input.focus(); // Open hote hi cursor input me aa jaye
    }
}
...
// Update your existing search listener to close overlay after typing
document.getElementById('searchInput').addEventListener('input', (e) => {
    const query = e.target.value.trim();
    if (query.length > 2) {
        currentEndpoint = `/search/multi?query=${encodeURIComponent(query)}`;
        currentPage = 1;
        fetchMovies(currentEndpoint);
        // Optional: Search start hone pe overlay hide karna ho toh:
